...
class Detectors(object):
    #Constructor
    def __init__(self, device, model, cpu_extension, plugin_dir,is_async_mode,number_infer_requests):
        self.cpu_extension = cpu_extension
        self.plugin_dir = plugin_dir
        self.is_async_mode = is_async_mode
        self.device=device
        self.model=model
        self.number_infer_requests=number_infer_requests
    ...
    def initialise_inference(self):
        ...
        print(self.number_infer_requests)
        exec_net = plugin.load(network=net, num_requests=self.number_infer_requests)
...
class Processor(object):
    ...
    def process_frame(self,cur_request_id,next_request_id,frame,is_async_mode,frame_count):
        
        res_list=[]
        '''
        required_inference_requests_were_executed= False
        steps_count=len(frame_queue)-1
        number_infer_requests=steps_count
        current_inference = 0
        previous_inference = 1 - len(frame_queue)
        step=0
        print("Entered")
        while not required_inference_requests_were_executed or step<steps_count:
            in_frame = cv2.resize(frame_queue[step], (self.w, self.h))
            in_frame = in_frame.transpose((2, 0, 1))  # Change data layout from HWC to CHW
            in_frame = in_frame.reshape((self.n, self.c, self.h, self.w))
            print("Entered 0")
            self.exec_net.start_async(request_id=current_inference, inputs={self.input_blob: in_frame})
            
            if previous_inference >= 0:
                print("Entered 1")
                status = self.exec_net.requests[current_inference].wait(-1)
                if status is not 0:
                    raise Exception("Infer request not completed successfully")
                else:
                    print("Entered 2")
                    res = self.exec_net.requests[current_inference].outputs[self.out_blob]
                    res_list.append(res)
